...
#include "A1.h"
...
#if defined(A1__PC)
    #define A1_SetP4PinDriveMode(shift, mode)  \
    do { \
        A1_PC =   (A1_PC & \
                                (uint32)(~(uint32)(A1_DRIVE_MODE_IND_MASK << \
                                (A1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (A1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define A1_SetP4PinDriveMode(shift, mode)  \
        do { \
            A1_USBIO_CTRL_REG = (A1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(A1_DRIVE_MODE_IND_MASK << \
                                    (A1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (A1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		A1_SetP4PinDriveMode(A1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(A1_DR & (uint8)(~A1_MASK));
    drVal = (drVal | ((uint8)(value << A1_SHIFT) & A1_MASK));
    A1_DR = (uint32)drVal;
}
...
uint8 A1_Read(void)
{
    return (uint8)((A1_PS & A1_MASK) >> A1_SHIFT);
}
...
{
    return (uint8)((A1_DR & A1_MASK) >> A1_SHIFT);
}
...
void A1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  A1_INTCFG & (uint32)(~(uint32)position);
    A1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(A1_INTSTAT & A1_MASK);
	A1_INTSTAT = maskedStatus;
    return maskedStatus >> A1_SHIFT;
}
